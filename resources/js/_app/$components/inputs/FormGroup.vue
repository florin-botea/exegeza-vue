<template>
	<div class="relative flex flex-wrap items-center">
		<label v-if="$attrs.id" :for="$attrs.id" class="inline-block pr-1 font-semibold" :class="labelPosition">
			{{label}}
		</label>
		<slot>
			<input v-bind="$attrs" :class="inputClass" class="flex-1 shadow border border-gray-600 appearance-none border rounded leading-tight focus:outline-none focus:shadow-outline w-full rounded" :required="required">
		</slot>
