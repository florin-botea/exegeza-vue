...
export default class Update extends Action {
  ...
  static async call ({ state, commit }, params = {}) {
    ...
    const request = axios.post(endpoint, {_method:method, data:params.data});
	//am adaugat niste hook-uri inainte si dupa ce se face request. le pot defini in model
	let goodToGo = true
	if (model.observer && model.observer.beforeUpdate){
		goodToGo = await model.observer.beforeUpdate( model.find(params.params.id || data.id) )
	}
	if (goodToGo){
  ...
  static onSuccess(model, params, data) {
	...
	//am adaugat niste hook-uri inainte si dupa ce se face request. le pot defini in model
	if (model.observer && model.observer.updated){
		model.observer.updated( model.find(params.params.id || data.id) )
	}
	//
  }
